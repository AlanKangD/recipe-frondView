...
export default function SignatureList() {
  const scrollLeft = () => {
    const container = document.getElementById("signature-slider")
    if (container) {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">                                                             
            <div className="text-center sm:text-left">
              ...
              <p className="text-base md:text-lg text-[#8B4513]/80 leading-relaxed font-sans max-w-2xl">                                                        
                저희만의 레시피의 시그니처 요리로 고민 없이 맛있는 식탁을 차려보세요.                                                                             
              </p>
            </div>
            <Link
              href="/signatures"
        ...
        <div className="relative">
          ...
          <div className="hidden lg:grid lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">                                                                         
            {signatures.map((signature) => (
              <SignatureCard key={signature.id} id={signature.id} title={signature.title} image={signature.image} />                                                 
            ))}
          </div>
          ...
          <div className="lg:hidden relative">
            ...
            >
              {signatures.map((signature) => (
                <div key={signature.id} className="snap-start flex-shrink-0">
                  <SignatureCard id={signature.id} title={signature.title} image={signature.image} />                                                               
                </div>
              ))}
            </div>
